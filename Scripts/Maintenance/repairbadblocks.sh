...
echo "CAUTION: Use at your own risk, repair attempts are non destructive but a mechanic disk may totally fail under 
stress."
...
echo "    Makes a list of all bad sectors with [badblock], then read the sectors to a tmp file, zero fill them, and 
write the data back from the tmp file."
echo ""
echo "hdparm repair:"
echo "    Makes a list of all bad sectors with [badblock], then reads them with hdparm, and tries to write them on 
read failure."
echo ""
echo "dd dumb repair:"
echo "    Same as dd intelligent repair, but instead of reading a list of bad sectors, proceeds for all sectors or a 
sector span."
echo ""
echo "Procedure explanation:"
echo ""
echo "Whenever a block of 4KB (8x512B sectors) is written to a disk, the disk firmware reallocates the sector if the 
sector is marked bad."
echo "Warning: both dd intelligent repair and hdparm repair may not find bad sectors to repair if they are in 
\"current pending\" SMART status"
...
echo "/tmp/directory is a temporary directory containing sector data and bad blocks lists. Make sure it is a ramdrive 
or another disk than the one your're trying to repair."
exit 1
}
...
function ddIntelligentRepair {
	...
	while read $block; do
		...
		sector=$((block*8))
		dd if=$drive iflag=direct of="$tmp_dir/badblock.$sector.$(basename $drive)" bs=4096 count=1 skip=$sector > 
/dev/null
		dd if=/dev/zero of=$drive oflag=direct bs=4096 count=1 skip=$sector > /dev/null
		dd if="$tmp_dir/badblock.$block.$(basename $drive)" of=$drive oflag=direct bs=4096 count=1 skip=$sector > 
/dev/null
		if [ $? == 0 ]; then
...
function ddDumbRepair {
	...
	while [ $continue == true ]; do
		dd if=$drive iflag=direct of="$tmp_dir/badblock.tmp.$(basename $drive)" bs=4096 count=1 skip=$block > /dev/null 
2>&1
		read_result=$?
		dd if=/dev/zero of=$drive oflag=direct bs=4096 count=1 skip=$block > /dev/null 2>&1
		dd if="$tmp_dir/badblock.tmp.$(basename $drive)" of=$drive oflag=direct bs=4096 count=1 skip=$block > /dev/null 
2>&1
		block=$((block+1))
